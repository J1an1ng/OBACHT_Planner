...
from source.crmonitor.common.helper import merge_dicts_recursively
...
def plot_rule_visualization(
    ...
    :param visualization_config: user-defined configurations of visualization
    ...
    for i in range(nr_rules):
        ...
        pred_result_dict[rule_evaluator_list[i]._rule.name] = (
    ...
    lanelet_network_draw_params = LaneletNetworkParams(
        time_begin=time_step,
        traffic_light=TrafficLightParams(draw_traffic_lights=False),
        traffic_sign=TrafficSignParams(draw_traffic_signs=False),
        lanelet=LaneletParams(
            fill_lanelet=True, facecolor="#c7c7c7", draw_linewidth=0.5, show_label=False
        ),
    )
    scenario.lanelet_network.draw(renderer, draw_params=lanelet_network_draw_params)
    # new
    ...
    for i in plot_veh_ids:
        if i != ego_vehicle_id:
            ...
            # )
            ego_dict = merge_dicts_recursively(
            ...
            scenario.obstacle_by_id(ego_vehicle_id).draw(
                renderer, draw_params=ego_draw_params
            )
