...
def load_scenario_and_planning_problem(
    path_scenario, idx_planning_problem: Optional[int] = None
) -> Tuple[Scenario, PlanningProblem, PlanningProblemSet]:
    """
    Loads a scenario and planning problem from the configurations.
    ...
    if idx_planning_problem is not None:
        try:
            planning_problem = planning_problem_set.find_planning_problem_by_id(
                idx_planning_problem
            )
        except KeyError:
            raise KeyError(
                f"<ReactivePlannerConfiguration.update()>:"
...
def retrieve_desired_velocity_from_pp(planning_problem: PlanningProblem):
    """Simple approach which retrieves average velocity from goal configurations"""
...
def shift_orientation(
    trajectory: Trajectory, interval_start=-np.pi, interval_end=np.pi
):
