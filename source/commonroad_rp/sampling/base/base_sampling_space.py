...
from source.commonroad_rp.polynomial_trajectory import QuarticTrajectory, QuinticTrajectory
from source.commonroad_rp.trajectories import TrajectorySample
...
class SamplingSpace(ABC):
    ...
    def __init__(self, num_sampling_levels: int, dt: float, horizon: float):
    ...
    @abstractmethod
    def _get_lon_samples(
        self, level_sampling: int, longitudinal_mode: str, **kwargs
    ) -> Iterable[float]:
    ...
    def generate_trajectories_at_level(
        ...
        # time_samples = sorted(self._get_time_samples(level_sampling))
        # iterate over time samples
        # for t in time_samples[:-1]:
        for t in self._get_time_samples(level_sampling):
            # iterate over longitudinal samples
            for lon_sample in self._get_lon_samples(
                level_sampling, longitudinal_mode, t_sample=t
            ):
                ...
                if trajectory_long.coeffs is not None:
                    ...
                    # print(f"lon coeffs{trajectory_long.coeffs}")
                    for d in self._get_lat_samples(
                        level_sampling,
                        x_0_lat=x_0_lat,
                        ...
                        if trajectory_lat.coeffs is not None:
                            ...
                            # print(f"lat coeffs{trajectory_lat.coeffs}")
    ...
    def _generate_lon_trajectory(
        ...
        if mode == "velocity_keeping":
            ...
            return QuarticTrajectory(
                tau_0=0, delta_tau=delta_tau, x_0=x_0, x_d=end_state_lon
            )
        elif mode == "stopping":
            ...
            raise AttributeError(
                f"<SamplingSpace>: specified longitudinal mode {mode} is invalid."
            )

    @staticmethod
    def _generate_lat_trajectory(
        delta_tau: float, x_0: np.ndarray, x_d: np.ndarray
    ) -> QuinticTrajectory:
    ...
    def generate_post_d_optimization_trajectories(
        self,
        current_d: float,
        x_0_lat: np.ndarray,
        lon_tra: Union[QuinticTrajectory, QuarticTrajectory],
    ) -> List[TrajectorySample]:
        """
        Generates a set of trajectories within the sampling space for a given sampling level.
        Interface method to the reactive planner to generate the trajectory set for the respective sampling space
        ...
        # initialize trajectory list
        list_trajectories = list()
        d_min, d_max = current_d - 1, current_d + 1
        samples = np.linspace(d_min, d_max, 50)
        print(samples)
        post_optimization_d = set(samples)
        # iterate over time samples
        # if trajectory_long.coeffs is not None:
        # iterate over lateral samples
        for d in post_optimization_d:
            # lateral end state (zero lat velocity & acceleration)
            ...
            if trajectory_lat.coeffs is not None:
                # add combined trajectory sample to output list
                list_trajectories.append(
                    TrajectorySample(self.horizon, self.dt, lon_tra, trajectory_lat)
                )
