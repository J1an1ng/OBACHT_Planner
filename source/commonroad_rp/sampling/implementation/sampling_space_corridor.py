...
from .sampling_domain import TimeSampling
...
class CorridorSampling(SamplingSpace):
    ...
    def __init__(self, config: ReactivePlannerConfiguration):
        ...
        self.samples_t = TimeSampling(
            config.sampling.t_min, self.horizon, self._num_sampling_levels, self.dt
        )
    ...
    def set_corridor(self, corridor: DrivingCorridor):
        ...
        for time_idx, connected_reach_set in self._corridor.items():
            position_interval = util_reach_operation.lon_interval_connected_set(
                connected_reach_set
            )
            velocity_interval = (
                util_reach_operation.lon_velocity_interval_connected_set(
                    connected_reach_set
                )
            )
            self._dict_lon_velocity_intervals[time_idx] = [
                velocity_interval[0],
                velocity_interval[1],
            ]
            self._dict_lon_position_intervals[time_idx] = [
                position_interval[0],
                position_interval[1],
            ]
    ...
    def set_t_sampling(self, t_min):
        ...
        self.samples_t = TimeSampling(
            t_min, self.horizon, self._num_sampling_levels, self.dt
        )
    ...
    def set_dict_number_of_samples(
        self, n_min: int = 3, dict_level_to_num_samples: dict = None
    ):
        """
        ...
        if dict_level_to_num_samples is not None:
            for level in range(self.num_sampling_levels):
                assert (
                    level in dict_level_to_num_samples.keys()
                ), f"input does not contain sampling level:{level}"
        else:
            n = n_min
    ...
    def _get_lon_samples(
        ...
        if longitudinal_mode == "velocity_keeping":
            ...
            raise AttributeError(
                f"<CorridorSampling>: specified longitudinal mode {longitudinal_mode} is"
                f"invalid."
            )
    ...
    def _get_lat_samples(self, level_sampling: int, **kwargs) -> Iterable[float]:
        ...
        lat_connected_sets = util_reach_operation.determine_connected_components(
            reachsets_overlap
        )
        ...
        for lat_con_set in lat_connected_sets:
            ...
            if lat_interval[0] < 0 < lat_interval[1]:
                ...
                d_samples_set: Set[float] = set(
                    np.linspace(lat_interval[0], lat_interval[1], num_samples)
                ).union({0})
            else:
                d_samples_set: Set[float] = set(
                    np.linspace(lat_interval[0], lat_interval[1], num_samples)
