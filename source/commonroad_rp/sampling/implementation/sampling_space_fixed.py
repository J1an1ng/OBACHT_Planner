...
class FixedIntervalSampling(SamplingSpace):
    ...
    def __init__(self, config: ReactivePlannerConfiguration):
        super(FixedIntervalSampling, self).__init__(
        ...
        # print(f"time:{config.planning.time_steps_computation}")
        # initialize and pre-compute samples in t, d, v, s domains
        self.samples_t = TimeSampling(
            config_sampling.t_min, self.horizon, self._num_sampling_levels, self.dt
        )
        logger.debug(self.samples_t)
        self.samples_d = PositionSampling(
            config_sampling.d_min,
            config_sampling.d_max,
            self._num_sampling_levels,
            config_sampling.pos_init_samples,
        )
        self.samples_v = VelocitySampling(
            config_sampling.v_min,
            config_sampling.v_max,
            self._num_sampling_levels,
            config_sampling.vel_init_samples,
        )
        self.samples_s = PositionSampling(
            config_sampling.s_min,
            config_sampling.s_max,
            self._num_sampling_levels,
            config_sampling.pos_init_samples,
        )

        # print(f"self._num_sampling_levels is {self._num_sampling_levels} ")
    ...
    def set_d_sampling(self, d_min, d_max):
        ...
        self.samples_d = PositionSampling(
            d_min, d_max, self._num_sampling_levels, self.samples_d._num_init_samples
        )

    def set_v_sampling(self, v_min, v_max):
        """Set samples in v domain of sampling space"""
        self.samples_v = VelocitySampling(
            v_min, v_max, self._num_sampling_levels, self.samples_v._num_init_samples
        )

    def set_s_sampling(self, s_min, s_max):
        """Set samples in s domain of sampling space"""
        self.samples_s = PositionSampling(
            s_min, s_max, self._num_sampling_levels, self.samples_s._num_init_samples
        )
    ...
    def _get_lon_samples(self, level_sampling, longitudinal_mode: str, **kwargs):
        ...
        if longitudinal_mode == "velocity_keeping":
            ...
            raise AttributeError(
                f"<FixedIntervalSampling>: specified longitudinal mode {longitudinal_mode} is"
                f"invalid."
            )
    ...
    # def _get_lon_samples_post(self, level_sampling, longitudinal_mode: str, **kwargs):
    #     """Returns longitudinal samples for the fixed sampling space"""
    #     if longitudinal_mode == "velocity_keeping":
    #         return self.samples_v.samples_at_level(level_sampling)
    #     elif longitudinal_mode == "stopping":
