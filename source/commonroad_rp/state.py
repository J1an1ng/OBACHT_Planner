...
import numpy as np
from commonroad.scenario.state import FloatExactOrInterval, InitialState, KSState
...
class ReactivePlannerState(KSState):
    ...
    * Extends KSState attributes by acceleration and yaw rate
    ...
    def __repr__(self):
        return (
            f"(time_step={self.time_step}, position={self.position},steering_angle={self.steering_angle}, "
    ...
    def shift_state_to_center(
        cls, state: "ReactivePlannerState", wb_rear_axle: float
    ...
    def create_from_initial_state(
        cls, initial_state: InitialState, wheelbase: float, wb_rear_axle: float
    ) -> "ReactivePlannerState":
        """
        ...
        * shifting position from center to rear axle
        ...
        :param wb_rear_axle: distance between rear-axle and vehicle center
        """
        assert type(initial_state) == InitialState, (
            "<ReactivePlannerState.create_from_initial_state()>: Input must be"
        ...
        initial_state_shifted = initial_state.translate_rotate(
            np.array(
                [
                    -wb_rear_axle * np.cos(orientation),
                    -wb_rear_axle * np.sin(orientation),
                ]
            ),
            0.0,
        )
        ...
        # add steering angle
        x0_planner.steering_angle = np.arctan2(
            wheelbase * x0_planner.yaw_rate, x0_planner.velocity
        )
