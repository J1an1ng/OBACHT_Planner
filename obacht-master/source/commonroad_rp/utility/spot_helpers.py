...
def spot_setup_scenario(
    scenario: Scenario,
    planning_problem: PlanningProblem,
    spot_scenario_id: int = 1,
...
def spot_compute_occupancy_prediction(
    scenario: Scenario,
    spot_scenario_id,
    start_time_step: int,
    end_time_step: int,
    ...
    print(
        "Calling SPOT prediction with ",
        num_threads,
        "threads and for time step interval [",
        start_time_step,
        ", ",
        end_time_step,
        "] with step size of ",
    ...
    obstacles_occupancy = spot.doOccupancyPrediction(
        spot_scenario_id, 0.0, dt, spot_horizon, num_threads
    )
    ...
    for cpp_obstacle in obstacles_occupancy:
        ...
        for vertices_at_time_step in cpp_obstacle[1]:
            ...
            while j < len(vertices_at_time_step[1]):
                compare_vertice = vertices_at_time_step[1][
                    b
                ]  # first vertice of next polygon
                if (
                    compare_vertice[0] == vertices_at_time_step[1][j][0]
                    and compare_vertice[1] == vertices_at_time_step[1][j][1]
                ):
                    if (j + 1) - b < 3:  # polygon has less than 3 vertices
                        ...
                        shape_obj = Polygon(
                            np.array(vertices_at_time_step[1][b : j + 1])
                        )
                ...
                j += 1
            assert b == j - 1, (
                "Last polygon not closed (at time_step = ",
                i,
                ", b = ",
                b,
            )
        ...
        scenario.dynamic_obstacles[k].prediction = SetBasedPrediction(
            start_time_step + 1, occupancy_list[0:]
        )
        k += 1
...
def spot_update_init_state_obstacles(scenario: Scenario, time_step):
    ...
    for dyn_obstacle in scenario.dynamic_obstacles:
        assert isinstance(
            dyn_obstacle.prediction, TrajectoryPrediction
        ...
        if dyn_obstacle.initial_state.time_step > time_step:
            ...
            try:
                pred_init_time_step = (
                    dyn_obstacle.prediction.trajectory.initial_time_step
                )
                dyn_obstacle.initial_state = (
                    dyn_obstacle.prediction.trajectory.state_list[
                        time_step - pred_init_time_step
                    ]
                )
            except IndexError:
...
class SpotManager:
    ...
    def __init__(
        self, spot_update_dict: dict, config: ReactivePlannerConfiguration = None
    ):
    ...
    def reset(
        self, config: ReactivePlannerConfiguration, curr_global_time_step: int = None
    ):
        ...
        self._curr_global_time_step = (
            curr_global_time_step if curr_global_time_step is not None else None
        )
    ...
    def predict(self):
        ...
        self._scenario = spot_update_init_state_obstacles(
            self._scenario, self._curr_global_time_step
        )
        # register scenario
        spot_setup_scenario(
            self._scenario,
            self._planning_problem,
            self.spot_scenario_id,
            self.spot_update_dict,
        )
        # do actual prediction
        spot_compute_occupancy_prediction(
            self._scenario,
            self.spot_scenario_id,
