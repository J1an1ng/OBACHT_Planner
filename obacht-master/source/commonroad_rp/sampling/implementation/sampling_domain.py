...
class VelocitySampling(Sampling):
    ...
    def __init__(
        self, low: float, up: float, num_sampling_levels: int, num_init_samples: int = 3
    ):
    ...
    def _sample(self):
        ...
        for i in range(self.num_sampling_levels):
            ...
            # new
            # print(f"[VelocitySampling] Level {i}, n={n}, samples={sorted(self._dict_level_to_sample_set[i])}")
            n = (n * 2) - 1
...
class PositionSampling(Sampling):
    ...
    def _sample(self):
        ...
        for i in range(self.num_sampling_levels):
            ...
            # print(f"[PositionSampling] Level {i}, n={n}, samples={sorted(self._dict_level_to_sample_set[i])}")
            n = (n * 2) - 1
...
class TimeSampling(Sampling):
    ...
    def __init__(self, low: float, up: float, num_sampling_levels: int, dt: float):
        self.dT = dt
        assert low >= 2 * self.dT, (
            "<TimeSampling: lower bound of time sampling must be greater-equal than the given"
            "time step>"
        )
    ...
    def _sample(self):
        for i in range(self.num_sampling_levels):
            ...
            samp = set(
                np.arange(self.low, round(self.up + self.dT, 2), step_size * self.dT)
            )
            samp.discard(round(self.up + self.dT, 2))
            self._dict_level_to_sample_set[i] = samp
            # new
            # print(f"[TimeSampling] Level {i}, step_size={step_size}, samples={sorted(samp)}")
