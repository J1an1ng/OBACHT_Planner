...
__author__ = "Peter Kocsis, Edmond Irani Liu, Youran Wang"
__copyright__ = "TUM Cyber-Physical System Group"
__credits__ = []
__version__ = "0.1"
__maintainer__ = "Youran Wang"
__email__ = "youran.wang@tum.de"
...
def simulate_without_ego(
    ...
    scenario_file = os.path.join(
        interactive_scenario_path, f"{conf.scenario_name}.cr.xml"
    )
    scenario, planning_problem_set = CommonRoadFileReader(scenario_file).open()

    scenario_wrapper = ScenarioWrapper()
    scenario_wrapper.sumo_cfg_file = os.path.join(
        interactive_scenario_path, f"{conf.scenario_name}.sumo.cfg"
    )
    ...
    simulated_scenario_without_ego, _ = simulate_scenario(
        SimulationOption.WITHOUT_EGO,
        conf,
        scenario_wrapper,
    ...
    if create_video:
        create_video_for_simulation(
            simulated_scenario_without_ego,
            output_folder_path,
...
def simulate_with_solution(
    ...
    if create_video:
        create_video_for_simulation(
            scenario_with_solution,
            output_folder_path,
            planning_problem_set,
            ego_vehicles,
            SimulationOption.SOLUTION.value,
        )
...
def simulate_with_planner(
    ...
    scenario_wrapper.initialize(
        conf.scenario_name,
        os.path.join(interactive_scenario_path, f"{conf.scenario_name}.sumo.cfg"),
        os.path.join(interactive_scenario_path, f"{conf.scenario_name}.cr.xml"),
    )
    scenario_wrapper.initial_scenario = scenario

    scenario_with_planner, ego_vehicles = simulate_scenario(
        SimulationOption.MOTION_PLANNER,
        conf,
        scenario_wrapper,
        interactive_scenario_path,
        num_of_steps=conf.simulation_steps,
    ...
    if create_video:
        create_video_for_simulation(
            ...
            planning_problem_set,
            ego_vehicles,
            SimulationOption.MOTION_PLANNER.value,
        )
...
def load_sumo_configuration(interactive_scenario_path: str) -> DefaultConfig:
    with open(
        os.path.join(interactive_scenario_path, "simulation_config.p"), "rb"
    ) as input_file:
...
def check_trajectories(
    solution: Solution, pps: PlanningProblemSet, config: DefaultConfig
):
    assert (
        len(set(solution.planning_problem_ids) - set(pps.planning_problem_dict.keys()))
        == 0
    ), (
    ...
    for s in solution.planning_problem_solutions:
        if s.trajectory.final_state.time_step < config.simulation_steps:
            raise ValueError(
                f"The simulation requires {config.simulation_steps} "
                f"states, but the solution only provides"
                f"{s.trajectory.final_state.time_step} time steps!"
            )


def create_video_for_simulation(
    scenario_with_planner: Scenario,
...
def simulate_scenario(
    ...
    if use_sumo_manager:
        ...
        sumo_sim.send_sumo_scenario(conf.scenario_name, scenario_path)
    else:
    ...
    if mode is SimulationOption.WITHOUT_EGO:
        ...
        def run_simulation():
            ...
            ego_vehicle_pre = next(iter(ego_vehicles.values()))
            ego_vehicle_pre.initial_state.steering_angle = 0
            ego_vehicle_pre.initial_state.slip_angle = 0
            ...
            for step in range(num_of_steps):
                ...
                current_scenario = sumo_sim.commonroad_scenario_at_time_step(
                ...
                for idx, ego_vehicle in enumerate(ego_vehicles.values()):
                    ...
                    # print(ego_vehicle.current_state)
                    ########################################################################
                    # TODO: Please implement your own planner here.
                    ########################################################################
                    # ====== Example motion planner 1: Dummy planner that decelerates to full stop
            ...
            for i, state in enumerate(state_list):
                state.time_step = i + 1  # from time step=1 begin
            traj = Trajectory(
            ...
            ego_vehicle = next(iter(sumo_sim.ego_vehicles.values()))
            ego_vehicle._state_dict.clear()
            for state in traj.state_list:
                ego_vehicle._state_dict[state.time_step] = state
        run_simulation()
        ...
        def run_simulation():
            ...
            for time_step in range(num_of_steps):
                ...
                for idx_ego, ego_vehicle in enumerate(ego_vehicles.values()):
                    ...
                    trajectory_solution = solution.planning_problem_solutions[
                        idx_ego
                    ].trajectory
                    next_state = copy.deepcopy(
                        trajectory_solution.state_list[time_step]
                    )
                    next_state.time_step = 1
                    trajectory_ego = [next_state]
                    ego_vehicle.set_planned_trajectory(trajectory_ego)
    ...
    sumo_sim.stop()
    ego_vehicles = {
        list(planning_problem_set.planning_problem_dict.keys())[0]: ego_v
        for _, ego_v in sumo_sim.ego_vehicles.items()
    }
    ego_vehicle = next(iter(ego_vehicles.values()))

    print(
        "final state is shown as:", ego_vehicle.driven_trajectory.trajectory.final_state
    )
    if use_sumo_manager:
