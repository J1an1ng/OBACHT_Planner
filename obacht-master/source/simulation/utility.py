...
from source.commonroad_rp.utility.visualization import plot_final_trajectory


def visualize_scenario_with_trajectory(
    scenario: Scenario,
...
def save_solution(
    scenario: Scenario,
    planning_problem_set: PlanningProblemSet,
    ego_vehicles: Dict[int, EgoVehicle],
    ...
    for pp_id, ego_vehicle in ego_vehicles.items():
        ...
        state_initial = copy.deepcopy(
            planning_problem_set.planning_problem_dict[pp_id].initial_state
        )
        ...
        for state in ego_vehicle.driven_trajectory.trajectory.state_list:
            ...
            set_attributes_in_state_extra = set_attributes_state.difference(
                set_attributes_state_initial
            )
            if set_attributes_in_state_extra:
                for attribute in set_attributes_in_state_extra:
                    setattr(state_initial, attribute, 0)

            set_attributes_in_state_initial_extra = (
                set_attributes_state_initial.difference(set_attributes_state)
            )
        ...
        trajectory_full = Trajectory(
            initial_time_step=0, state_list=list_states_trajectory_full
        )
        pps.append(
            PlanningProblemSolution(
